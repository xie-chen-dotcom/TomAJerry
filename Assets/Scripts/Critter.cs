...
public class Critter : MonoBehaviour
{
    ...
    void ReceiveAnAttack(int damage)
    {
        ...
        {
            IsDead = true;
            EventManager.Instance.OnCritterDead(1);//角色的生命值小于0，即死亡时，调用角色死亡事件OnCritterDead().
            CritterIsDeath();
        }
    }
    /// <summary>
    /// 死亡函数-当蜗牛的生命值小于零时，在蜗牛的右前方实例化一个掉落物品。同时，销毁蜗牛物体。
    /// </summary>
    public void CritterIsDeath()
    {
        ...
        GameObject prefab = critterSO.GetRandomItem().Prefab;//2.用蜗牛的数据文件随机一个掉落物品
